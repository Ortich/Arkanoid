...
public class Arkanoid extends acm.program.GraphicsProgram{
	...
	public void run(){
		...
		//Para que reconozca bien cuando pintar niveles, pondremos un if, para que los pinte si quedan vidas. Cuando acabe el nivel
		//reposiciona la bola  y su velocidad para empezar de nuevo
		...
		//	startGame();
		//	}
		//ball_1.setLocation(getWidth()/2-bar_1.getWidth(), getHeight()*0.70-ball_1.getHeight());
		//speed = 10;
	...
	private void level_02(){
		...
		for(int j=0; j<brickNumber; j++){//Luego con los dos for pinta la piramide
			for(int i=0; i<brickNumber; i++){
				Brick myBrick = new Brick(0+brickWidth*i, 0+brickHeight*j, brickWidth, brickHeight, Color.MAGENTA);
				add(myBrick);//Una vez determinada la posicion,añade el brick al juego
				this.brickNumber+=1;//Añade un brick al contador de bricks del juego
				pause(10);//Y añade un pause para que se vea mas bonito
			}
